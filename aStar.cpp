...
void cAStar::Build(cBotBase& bot)
{
	...
	std::vector <float> lowestCost; //For lowest cost with heuristic value
	std::vector <float> lowestCostX; //For X coordinate
	std::vector <int> lowestCostY; //For Y coordinate
	...
	float heuristic = fabs(gTarget.PositionX() - bot.PositionX()) + fabs(gTarget.PositionY() - bot.PositionY()); 
	...
	while (!closed[gTarget.PositionX()][gTarget.PositionY()] && lowestCost.size() != 0) //While player's node has not been closed or the cost vector is not empty
	{
		...
		//if the block is not closed and the new cost won't be higher than it already is
		if (closed[tempX][tempY + 1] == false && gLevel.isValid(tempX, (tempY + 1)))
		{
			...
			{
				heuristic = fabs(gTarget.PositionX() - tempX) + fabs(gTarget.PositionY() - (tempY + 1));
		...
		if (closed[tempX - 1][tempY] == false && gLevel.isValid((tempX - 1), tempY))
		{
			...
			{
				heuristic = fabs(gTarget.PositionX() - (tempX - 1)) + fabs(gTarget.PositionY() - tempY);
		...
		if (closed[tempX - 1][tempY + 1] == false && gLevel.isValid((tempX - 1), (tempY + 1)))
		{
			...
			{
				heuristic = fabs(gTarget.PositionX() - (tempX - 1)) + fabs(gTarget.PositionY() - (tempY + 1));
	...
	path.push_back({ gTarget.PositionX(), gTarget.PositionY() });
	...
	while (!done)
	{
		...
		path.push_back({ nextClosedX, nextClosedY }); //Add coordinates to a vector

		//If last item in a vector is bot's coordinates then finish the loop
		if ((path.back().x == bot.PositionX()) && (path.back().y == bot.PositionY()))
		{
			done = true;
		}
