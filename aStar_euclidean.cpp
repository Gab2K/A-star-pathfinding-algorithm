...
void cAStar_euclidean::Build(cBotBase& bot)
{
	...
	float heuristic = sqrt(pow(gTarget.PositionX() - bot.PositionX(), 2) + pow(gTarget.PositionY() - bot.PositionY(), 2));
	...
	while (!closed[gTarget.PositionX()][gTarget.PositionY()] && lowestCost.size() != 0) //While player's node has not been closed or the cost vector is not empty
	{
		...
		if (closed[tempX][tempY + 1] == false && gLevel.isValid(tempX, (tempY + 1)))
		{
			...
			{
				heuristic = sqrt(pow(gTarget.PositionX() - tempX, 2) + pow(gTarget.PositionY() - (tempY + 1), 2));
		...
		if (closed[tempX - 1][tempY] == false && gLevel.isValid((tempX - 1), tempY))
		{
			...
			{
				heuristic = sqrt(pow(gTarget.PositionX() - (tempX - 1), 2) + pow(gTarget.PositionY() - tempY, 2));
		...
		if (closed[tempX - 1][tempY + 1] == false && gLevel.isValid((tempX - 1), (tempY + 1)))
		{
			...
			{
				heuristic = sqrt(pow(gTarget.PositionX() - (tempX - 1), 2) + pow(gTarget.PositionY() - (tempY + 1), 2));
...
cAStar_euclidean gAStar_euclidean;